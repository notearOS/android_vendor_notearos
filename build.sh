...
function showHelpAndExit {
        ...
        echo -e "${CLR_BLD_BLU}  -v, --variant         notearOS variant - Can be dev, or release${CLR_RST}"
...
while true; do
    case "$1" in
        ...
        -v|--variant|v|variant) NOTEAROS_VARIANT="$2"; shift;;
...
if [ ! -d "$DIR_ROOT/vendor/notearos" ]; then
        echo -e "${CLR_BLD_RED}error: insane root directory ($DIR_ROOT)${CLR_RST}"
        exit 1
fi

# Setup PA variant if specified
if [ $NOTEAROS_VARIANT ]; then
    NOTEAROS_VARIANT=`echo $NOTEAROS_VARIANT |  tr "[:upper:]" "[:lower:]"`
    if [ "${NOTEAROS_VARIANT}" = "release" ]; then
        export NOTEAROS_BUILDTYPE=RELEASE
    elif [ "${NOTEAROS_VARIANT}" = "dev" ]; then
        unset NOTEAROS_BUILDTYPE
    else
...
NOTEAROS_DISPLAY_VERSION="$(cat $DIR_ROOT/vendor/notearos/config/version.mk | grep 'NOTEAROS_VERSION_FLAVOR := *' | sed 's/.*= //') \
$(cat $DIR_ROOT/vendor/notearos/config/version.mk | grep 'NOTEAROS_VERSION_CODE := *' | sed 's/.*= //')"
...
echo -e "${CLR_BLD_GRN}Building NOTEAROS $NOTEAROS_DISPLAY_VERSION for $DEVICE${CLR_RST}"
...
echo -e ""
NOTEAROS_VERSION=$(lunch "notearos_$DEVICE-$BUILD_TYPE" | grep 'NOTEAROS_VERSION=*' | sed 's/.*=//')
lunch "notearos_$DEVICE-$BUILD_TYPE"
echo -e ""
...
# Build signed rom package if specified
    ...
    sign_target_files_apks -o -d $KEY_MAPPINGS \
        out/dist/notearos_$DEVICE-target_files-$FILE_NAME_TAG.zip \
        notearos-$NOTEAROS_VERSION-signed-target_files-$FILE_NAME_TAG.zip
    ...
    if [ "$DELTA_TARGET_FILES" ]; then
        ...
        ota_from_target_files -k $KEY_MAPPINGS/releasekey \
            ...
            notearos-$NOTEAROS_VERSION-signed-target_files-$FILE_NAME_TAG.zip \
            notearos-$NOTEAROS_VERSION-delta.zip
        checkExit
    fi
    ...
    if [ "$FLAG_IMG_ZIP" = 'y' ]; then
        img_from_target_files \
            notearos-$NOTEAROS_VERSION-signed-target_files-$FILE_NAME_TAG.zip \
            notearos-$NOTEAROS_VERSION-signed-image.zip
        checkExit
    fi
# Build rom package
    ...
    cp -f $OUT/notearos_$DEVICE-ota-$FILE_NAME_TAG.zip $OUT/notearos-$NOTEAROS_VERSION.zip
    cp -f $OUT/notearos_$DEVICE-img-$FILE_NAME_TAG.zip $OUT/notearos-$NOTEAROS_VERSION-image.zip
